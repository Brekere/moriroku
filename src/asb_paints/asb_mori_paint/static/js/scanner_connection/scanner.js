...
let id_component_ = parseInt(localStorage.getItem('id_component'))
/* 
    temperatura: 0
    humedad: 1
    ambos: 2
...
$(BarcodesScanner).on('scan', async function(e){
    ...
    if(r == 15){
        ...
        console.log("Respuesta recibida",response_data.rest)
        if(response_data.rest == 'SUCCESS'){
            var job_position = parseInt(response_data.record.job_position)
            if(job_position == 0){
                $('#credentialsAssistantModal').modal('hide')
                console.log(struct_error_process)
                console.log(struct_error_container)
                await paintLabel(1, viscosity_obteined)
                await post_process_errors(struct_error_process_)
                await post_container_errors(struct_error_container)
                //preguntar si se va imprimir
...
$('#errorDBModal').on('show.bs.modal', () => {
    const timer = 5000
    setTimeout(() => {
        $('#errorDBModal').modal('hide')
    }, timer)
})